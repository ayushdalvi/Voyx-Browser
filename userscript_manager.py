...
(function() {{
    ...
    window.GM_notification = function(details) {{
        ...
        console.log('[GM Notification]', details.title || 'Notification', ':', details.text || '');
    }};
    ...
    // Clipboard
    window.GM_setClipboard = function(data) {{
...
"""
    ...
    def install_from_url(self, url):
        ...
        try:
            ...
            print(f"Error installing script from {url}: {e}")
        return False
    
    def open_manager_gui(self, parent=None):
        """Open the userscript manager GUI."""
        try:
            from simple_userscript_gui import SimpleUserscriptManager
            gui = SimpleUserscriptManager(self, parent)
            return gui
        except Exception as e:
            ...
            msg.setText(f"Could not open userscript manager: {str(e)}")
            msg.exec_()
            return None